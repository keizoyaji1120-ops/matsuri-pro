...
st.markdown("""
    ...
    .weight-val { font-weight: bold; font-size: 24px; color: #d63031; }
...
def calc_strategy_realtime(wind_spd, wind_dir, tide_factor, is_rising, temp, cloud, rain, target_depth_mode):
    ...
    base_depth = 45 
    if target_depth_mode == "30m": base_depth = 30
    ...
    # ★ 水深 × 天候 によるアレンジ (ケイムラ排除)
    if target_depth_mode == "30m":
        # 浅場: 光が届く
        if "オレンジ" in color:
            if cloud < 50: color = "クリアオレンジ / 蛍光オレンジ (光透過)" # 晴れ: 透けさせる
            else: color = "オレンジ / 赤オレ (膨張色)" # 曇り: そのまま
        elif "赤" in color:
            color = color.replace("赤", "クリアレッド") # 晴れ曇り問わずクリアレッド推奨
        elif "黒" in color or "ブラック" in color:
            color += " (スモーク/薄め)" # 浅場の黒は強すぎるためスモーク推奨
            
    elif target_depth_mode == "60m":
        # 深場: 光が減衰
        if "オレンジ" in color:
            if cloud < 50: color = "金オレ / オレンジゴールド (フラッシング)" # 晴れ: フラッシング
            else: color = "オレンジ (グロー) / チャート (視認性)" # 曇り: グロー・チャート
        elif "赤" in color:
