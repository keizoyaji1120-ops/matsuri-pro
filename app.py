...
# --- 定数 (観測点と時差補正) ---
# offset_min: その場所の潮が、基準点(ref_key)より「何分遅れて」やってくるか
JCG_POINTS = {
    "akashi": {
        "name": "明石海峡",
    ...
    # 【追加】小豆島・播磨灘エリア (明石データ +60分補正)
    "shodoshima": {
        "name": "小豆島 (播磨灘)",
        "lat": 34.480, "lon": 134.350, # 小豆島東側沖
        "url": None, # URLなし (明石を参照)
        "offset_min": 60, # 明石より約1時間遅れ
        "ref_key": "akashi"
    }
}
...
# --- 潮流計算 (ハイブリッド・時差補正対応 v13.0) ---
def get_hybrid_tide_data(target_datetime, now_datetime, port_info):
    """
    時差補正(offset_min)を考慮してデータを取得する。
    例: 小豆島(offset+60)で10時の予報が欲しい場合、明石の09時のデータを参照する。
    (明石で09時に起きた現象が、10時に小豆島に到達するため)
    """
    
    # 参照すべき元データの時間 (現在時刻 - オフセット)
    # 例: ターゲットが10:00, オフセット+60分(遅れ)なら、元データは09:00のものを見る
    ...
    if success:
        ...
        # 計算ロジックの場合も参照時間(ref_dt)の月齢・時間を使う
...
def calc_strategy_realtime(wind_spd, wind_dir, tide_factor, is_rising, temp, cloud, rain):
    ...
    # --- ネクタイ選定ロジック ---
    tie_size = "標準カーリー"
    maker_rec = "START: カーリー / がまかつ: 桜幻" 
    
    if tide_factor < 0.2:
        tie_size = "極細ショート / ストレート"
        maker_rec = "💡【ワーム推奨】FishArrow: フラッシュJ / Ecogear: 熟成アクア"
    elif temp < 12.0:
        tie_size = "極細ショートカーリー"
        maker_rec = "Jackall: フィネスカーリー / ササラボ: 極薄"
    elif 12.0 <= temp < 15.0 and is_synced:
        tie_size = "ショートストレート"
        maker_rec = "ダイワ: 中井チューン / 海遊: シングル"
    elif 6 <= datetime.datetime.now().month <= 8 and tide_factor > 0.5:
        tie_size = "フィッシュテール / ツイン"
        maker_rec = "ササラボ: 極厚 / Jackall: イカクロー"
    elif rain > 0.5 or tide_factor > 0.8:
        tie_size = "強波動ワイド / ビッグ"
        maker_rec = "Jackall: マスターカーリー / 海遊: WG"
    else:
    ...
    return weights_dict, color, tie_size, maker_rec, speed, tactic, is_synced, tide_dir_deg
...
def main():
    ...
    with st.spinner('解析中...'):
        ...
        if data:
            ...
            rec_weights, rec_color, rec_size, rec_maker, rec_speed, rec_tactic, is_synced, tide_dir_deg = calc_strategy_realtime(
            ...
            with col_b:
                ...
                <div class="rec-box" style="border-color: #e67e22; background-color: #fdf2e9;">
                    ...
                    <div class="maker-rec">💡例: {rec_maker}</div>
                </div>
                """, unsafe_allow_html=True)
