...
# --- 統計水温計算ロジック (v18.2: 旧アプリ相当の日割り・時間計算) ---
def calculate_historical_sst_precise(now_dt):
    # 1. 基準となる月別平均水温 (明石・瀬戸内海モデル)
    # 2月下旬が底、8月下旬がピーク
    monthly_temps = {
    ...
    # 2. 日割り計算 (線形補間)
    # 現在の月の中央値(15日)と、前月or翌月の中央値の間で今の位置を計算
    ...
    # 3. 時間変動 (日周変化)
    # 最低気温 06:00, 最高気温 15:00 と仮定したサインカーブ
    # 振幅は浅場で大きく、深場で小さいが、ここでは平均的に +/- 0.3度とする
    
    # hour: 0-23
    # peak at 15 (max), bottom at 3 (min) approx
