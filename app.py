...
st.set_page_config(page_title="魔釣Pro - Forecast", page_icon="⚓️")
...
def get_current_weather(lat, lon):
    ...
    # 【修正】hourlyに風速・風向を追加して未来の予報を取得できるようにする
    params = {
        "latitude": lat,
        "longitude": lon,
        ...
        "timezone": "Asia/Tokyo",
        "forecast_days": 2, # 日をまたぐ予報のために2日分取得
        "wind_speed_unit": "ms"
    }
...
def get_hybrid_tide_data(moon_age, hour, minute):
    ...
    if success:
        tide_dir_deg = 280 if "西" in dr_text else 100 
...
def main():
    st.markdown("""
        ...
        <p style='text-align: center; font-size: 14px; color: gray;'>
            JCG Official Data & Strategy v8.0
        </p>
    """, unsafe_allow_html=True)
    ...
    with st.spinner('解析中...'):
        ...
        if data:
            ...
            for i in range(1, 5):
                ...
                # 風データ (Hourlyから取得) - リストのインデックスは 0=0:00 なので f_h をそのまま使う
                # APIは48時間分などを返すが、インデックスオーバー防止
                if data["hourly"]["wind_speed_10m"] and len(data["hourly"]["wind_speed_10m"]) > f_h:
                    fw_spd = data["hourly"]["wind_speed_10m"][f_h]
                    fw_dir = data["hourly"]["wind_direction_10m"][f_h]
                    fw_card = deg_to_cardinal(fw_dir)
                    fw_code = data["hourly"]["weather_code"][f_h]
                    # 天気アイコン簡易判定
                    w_icon = "☀️"
                ...
                # 潮流データ (ハイブリッド)
                ft_fac, ft_rise, ft_knot, ft_off = get_hybrid_tide_data(moon_age, f_h, 0)
                if ft_fac < 0.1 and ft_knot < 0.5:
                    ft_text = "<span class='fc-tide-stop'>★転流 / 潮止まり</span>"
                else:
                    ft_dir = "西 (上げ)" if ft_rise else "東 (下げ)"
                    ft_text = f"{ft_dir} {ft_knot:.1f}kt"
                
                forecast_html += f"<tr><td class='fc-time'>{f_h}:00</td><td>{fw_text}</td><td>{ft_text}</td></tr>"
