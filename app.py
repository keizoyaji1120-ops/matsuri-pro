...
# --- 潮流計算 (ハイブリッド・日付判定強化) ---
def get_hybrid_tide_data(target_datetime, now_datetime):
    """
    JCGデータは「今日」のものしか取得できないため、
    日付が変わっている場合はJCGを使わず計算ロジックに倒す。
    """
    # 日付が同じかどうかチェック
    is_same_day = (target_datetime.day == now_datetime.day)
    
    # 1. JCGデータ取得 (日付が同じ場合のみ)
    success = False
    knot = 0.0
    dr_text = ""
    ...
    if success:
        ...
        return tide_factor, is_rising, knot, True # True=公式
    else:
        # 2. 計算ロジック (日付変更時 or 取得失敗時)
        # ターゲット日時の月齢を計算して渡す
        ...
        return tide_factor, is_rising, knot, False # False=推測
...
def calc_strategy_realtime(wind_spd, wind_dir, tide_factor, is_rising, temp, cloud, rain):
    ...
    tie_size = "標準カーリー"
    if tide_factor < 0.2:
        tie_size = "ワーム装着 (フラッシュJ等)"
    elif temp < 12.0:
        tie_size = "極細シングル (Start系)"
    elif 12.0 <= temp < 15.0 and is_synced:
        tie_size = "ショートストレート"
...
def main():
    st.markdown("""
        ...
        <p style='text-align: center; font-size: 14px; color: gray;'>
            JCG Official Data & Strategy v8.2
        </p>
    """, unsafe_allow_html=True)
    ...
    with st.spinner('解析中...'):
        ...
        if data:
            ...
            # 潮流 (現在は now を渡す)
            tide_factor, is_rising, real_knot, is_official = get_hybrid_tide_data(now, now)
            ...
            for i in range(1, 5):
                ...
                # APIのhourlyデータは0時始まりなのでインデックスを計算
                # 昨日の23時に実行して今日の0時~3時を取る場合などを考慮し、
                # 現在のインデックス(now.hour) + i で取得する
                target_idx = now.hour + i
                
                # インデックスが48時間(API仕様)を超えないかチェック
                ...
                # 潮流データ (日付またぎ対応版を呼ぶ)
                ft_fac, ft_rise, ft_knot, ft_off = get_hybrid_tide_data(f_time, now)
                ...
                if ft_fac < 0.1 and ft_knot < 0.5:
                    ...
                    ft_text = f"{ft_dir} {ft_knot:.1f}kt{tide_source}"
