...
st.markdown("""
    ...
    .fc-score-high { color: #e74c3c; font-weight: bold; } /* 高スコア用 */
    .fc-score-mid { color: #f39c12; font-weight: bold; } /* 中スコア用 */
    .fc-score-low { color: #95a5a6; } /* 低スコア用 */
...
def get_current_weather(lat, lon):
    ...
    # 【修正 v15.0】hourlyに 'rain' を追加して未来のスコア計算に使う
    params = {
        "latitude": lat,
        "longitude": lon,
...
def calc_strategy_realtime(wind_spd, wind_dir, tide_factor, is_rising, temp, cloud, rain):
    ...
    if tide_factor < 0.2:
        ...
        tie_size = "ショートストレート"
        maker_rec = "推奨例: ダイワ 中井チューン / 海遊 シングル"
    ...
    if rain >= 0.5:
        ...
        if is_nori_season:
            color = "海苔グリーン / コーラ (海苔パターン)"
        else:
            color = "レッドブラック / スモーク (スレ対策)"
    elif is_nori_season and cloud > 50:
        color = "コーラ / レッドゼブラ (海苔・シルエット)"
    elif temp < 12.0:
        if is_nori_season:
            color = "コーラ / マットブラック (低水温)"
        else:
            color = "ダークグリーン / マットブラック"
    elif cloud >= 90:
        color = "マジョーラゼブラ / 赤ゼブラ"
    elif 60 <= cloud < 90:
        color = "コーラ / 赤黒"
    ...
    with st.spinner('解析中...'):
        ...
        if data:
            ...
            for i in range(1, 5):
                ...
                f_score_html = f"<span class='{score_class}'>{score_icon} {f_score}</span>"
