...
def calc_strategy_realtime(wind_spd, wind_dir, tide_factor, is_rising, temp, cloud, rain, target_depth_mode, sunrise_dt, sunset_dt, current_dt, area_key):
    ...
    # ---------------------------------------------------------
    # v21.3: ベテラン向けウェイト調整 (深場を軽めに判定)
    # ---------------------------------------------------------
    base_depth = 45 
    if target_depth_mode == "15m": base_depth = 15
    elif target_depth_mode == "30m": base_depth = 30
    elif target_depth_mode == "60m": base_depth = 50 # 玄人の糸捌きを前提に60→50相当へ
    elif target_depth_mode == "80m": base_depth = 65 # 玄人の糸捌きを前提に80→65相当へ
    ...
    is_autumn = current_month in [9, 10, 11]

    # ---------------------------------------------------------
    # v21.3: 4大メーカー特化・ネクタイパターン判定
    # (ジャッカル, 海遊, START, sasalabo)
    # ---------------------------------------------------------
    tie_size = "標準カーリー"
    maker_rec = "推奨例: START カーリー / ジャッカル マスターカーリー" 

    if area_key == "naruto":
        # 【鳴門エリア】激流・ディープ・リアクション重視
    ...
    if rain > 0.5 or cloud >= 90:
        if "極細" in tie_size or "ショート" in tie_size:
            ...
            maker_rec = maker_rec.replace("極薄", "レギュラー").replace("ショートスキニー", "カーリー")

    # ---------------------------------------------------------
    # v21.3: ご当地・時間帯・天候クロス判定 カラーロジック
    # ---------------------------------------------------------
    is_mazume = False
    is_night = False
    ...
    if is_night:
        ...
        if rain >= 0.5:
            ...
            if area_key == "akashi":
                color = "海苔グリーン / コーラ (シルエット重視)" if is_nori_season else "レッドブラック / スモーク"
            elif area_key == "naruto":
                color = "レッドゼブラ / ダークレッド"
            else:
                color = "グリーン / ケイムラ (ナチュラル・エビ)"
        elif cloud >= 80:
            if area_key == "naruto": color = "グローゼブラ / オレンジゴールド"
            else: color = "マジョーラゼブラ / 赤ゼブラ"
        elif 30 <= cloud < 80:
            color = "シマシマオレンジ / 蛍光オレンジ"
        elif cloud < 30:
            if tide_factor > 0.4: color = "オレンジドット / クリアレッド"
            else: color = "ケイムラ / ピンク / クリアオレンジ"

        # 水深による最終調整 (光の透過度)
        if target_depth_mode in ["15m", "30m"] and cloud < 30 and not is_night and not is_mazume:
            if "オレンジ" in color: color = "クリアオレンジ / ケイムラオレンジ (光透過)" 
            elif "赤" in color: color = color.replace("赤", "クリアレッド") 
            elif "黒" in color or "コーラ" in color: color = "スモーク / クリアブラック (ステルス)"
                
        elif target_depth_mode in ["60m", "80m"] and not is_night:
            if "オレンジ" in color: color = "金オレ / オレンジゴールド (フラッシング)"
            elif "赤" in color: color += " (グローエッジ)"
            elif "グリーン" in color: color += " (チャート/ゴールド)"
