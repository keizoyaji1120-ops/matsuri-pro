...
st.set_page_config(page_title="魔釣Pro - The Maker", page_icon="⚓️")
...
def calc_strategy_realtime(wind_spd, wind_dir, tide_factor, is_rising, temp, cloud, rain):
    ...
    # --- ネクタイ選定ロジック (ワーム具体化 v11.2) ---
    tie_size = "標準カーリー"
    maker_rec = "START: カーリー / がまかつ: 桜幻" 
    
    if tide_factor < 0.2:
        # ★ワーム推奨 (具体名を追加)
        tie_size = "ワーム (フラッシング＆匂い)"
        maker_rec = "FishArrow: フラッシュJ / Ecogear: 熟成アクア"
    elif temp < 12.0:
        tie_size = "極細ショートカーリー"
...
def main():
    st.markdown("""
        ...
        <p style='text-align: center; font-size: 14px; color: gray;'>
            JCG Official Data & Strategy v11.2
        </p>
    """, unsafe_allow_html=True)
    ...
    with st.spinner('解析中...'):
        ...
        if data:
            ...
            if dist_km > 20: port_msg += f" (距離 {int(dist_km)}km ※参考値)"
            else: port_msg += " (JCG公式)"
