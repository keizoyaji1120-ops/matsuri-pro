...
def get_current_weather(lat, lon):
    ...
    # 【修正】wind_speed_unit=ms を追加して秒速で取得する
    params = {
        "latitude": lat,
        "longitude": lon,
...
def calc_strategy_realtime(wind_spd, wind_dir, tide_factor, is_rising, temp, cloud, rain):
    ...
    elif 30 <= cloud < 60: color = "オレンジゼブラ / オレンジ"
...
def main():
    st.markdown("""
        ...
        <p style='text-align: center; font-size: 14px; color: gray;'>
            JCG Official Data & Strategy v6.5
        </p>
    """, unsafe_allow_html=True)
    ...
    with st.spinner('解析中...'):
        ...
        if data:
            ...
            def get_style(target_code):
                base = "seat-cell"
                if target_code == seat_code:
                    return base + " seat-best"
                if seat_code == "m_center" and target_code in ["m_left", "m_right"]:
                    return base + " seat-best"
                if seat_code == "t_center" and target_code in ["t_left", "t_right"]:
                    return base + " seat-best"
                return base
