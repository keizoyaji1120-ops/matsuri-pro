...
# --- 水温確保用: 確実に海判定される沖合バックアップ座標 (v17.3追加) ---
RELIABLE_SST_POINTS = {
    "akashi": {"lat": 34.580, "lon": 135.000}, # 明石海峡の南側沖合
    "naruto": {"lat": 34.230, "lon": 134.700}, # 鳴門沖
    "tomogashima": {"lat": 34.350, "lon": 135.000}, # 友ヶ島北側
    "shodoshima": {"lat": 34.450, "lon": 134.300} # 播磨灘中央寄り
}
...
def get_current_weather(lat, lon):
    ...
    if sst_list and current_hour < len(sst_list) and sst_list[current_hour] is not None:
        has_sst = True
        base_data["sst_source"] = "local" # 現地データ成功
    
    # 水温が取れていない場合、沖合ポイントへリトライ
    if not has_sst:
        # 最寄りのバックアップポイントを探す
        _, _, port_key = get_nearest_port(lat, lon)
        backup = RELIABLE_SST_POINTS.get(port_key)
        
        if backup:
            # 沖合ポイントの天気データを取得
            backup_data = fetch_open_meteo(backup["lat"], backup["lon"])
            if backup_data:
                backup_sst_list = backup_data["hourly"].get("sea_surface_temperature", [])
                if backup_sst_list and current_hour < len(backup_sst_list) and backup_sst_list[current_hour] is not None:
                    # 沖合データで上書き
                    base_data["hourly"]["sea_surface_temperature"] = backup_sst_list
                    base_data["sst_source"] = "offshore" # 沖合補正成功
                else:
