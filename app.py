...
def get_current_weather(lat, lon):
    is_default_location = abs(lat - DEFAULT_LAT) < 0.001 and abs(lon - DEFAULT_LON) < 0.001
    fetch_lat = lat
    fetch_lon = lon
    
    if is_default_location:
        fetch_lat = RELIABLE_SST_POINTS['akashi']['lat']
        fetch_lon = RELIABLE_SST_POINTS['akashi']['lon']
    ...
    if sst_list and current_hour < len(sst_list) and sst_list[current_hour] is not None:
        has_sst = True
        base_data["sst_source"] = "search" if is_default_location else "local"
    
    if not has_sst and not is_default_location:
        search_offsets = [
...
def calc_strategy_realtime(wind_spd, wind_dir, tide_factor, is_rising, temp, cloud, rain, target_depth_mode, sunrise_dt, sunset_dt, current_dt):
    ...
    if sunrise_dt and sunset_dt:
        ...
        if (sr - datetime.timedelta(minutes=60)) <= current_dt <= (sr + datetime.timedelta(minutes=60)):
            is_mazume = True 
        elif (ss - datetime.timedelta(minutes=60)) <= current_dt <= (ss + datetime.timedelta(minutes=60)):
            is_mazume = True 
            
        if current_dt < (sr - datetime.timedelta(minutes=30)) or current_dt > (ss + datetime.timedelta(minutes=30)):
            is_night = True

    if is_mazume:
        color = "アカキン / オレキン / 緑金 (マズメ特化)"
    elif is_night:
    ...
    # v20.3: 表現修正 & キャスティング自動判定
    tactic = "バーチカル気味 (縦の釣り)"
    if is_synced and wind_spd > 3.0:
        tactic = "斜め引き (広範囲攻略)" # ドテラという言葉を排除
...
def main():
    ...
    # --- Windy Embed ---
    st.markdown("### 🗺️ 高度海況マップ (Windy Embed)")
    windy_html = f"""
    <div style="width: 100%; height: 350px; border-radius: 10px; overflow: hidden; box-shadow: 0 0 10px rgba(0,0,0,0.1);">
        <iframe width="100%" height="350" src="https://embed.windy.com/embed2.html?lat={lat}&lon={lon}&detailLat={lat}&detailLon={lon}&width=650&height=350&zoom=10&level=surface&overlay=wind&product=ecmwf&menu=&message=&marker=&calendar=now&pressure=&type=map&location=coordinates&detail=&metricWind=m%2Fs&metricTemp=%C2%B0C&radarRange=-1" frameborder="0"></iframe>
    </div>
    """
    components.html(windy_html, height=350)
    
    st.caption("※ Windyは世界最強の気象モデル(ECMWF)を視覚化します。風向きや波の予測にご活用ください。")
    ...
    with st.spinner('解析中...'):
        ...
        if data:
            ...
            rec_weight, rec_color, rec_size, rec_maker, rec_speed, rec_tactic, is_synced, tide_dir_deg = calc_strategy_realtime(
                wind_spd, wind_dir, tide_factor, is_rising, sst, cloud, rain, target_depth_mode, sunrise, sunset, now
            )
            ...
            <div class="rec-box">
                ...
                <div class="rec-content" style="font-size: 22px;">{rec_tactic}</div>
                <div style="margin-top: 5px; font-size: 14px;">推奨TGウェイト: <span class="weight-val">{rec_weight}g</span></div>
                <div class="captain-note">※流し方（縦/ドテラ）や重さは、必ず船長の指示に従ってください。</div>
            </div>
            """, unsafe_allow_html=True)
