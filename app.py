...
import streamlit.components.v1 as components # v20.0追加: Windy埋め込み用
import pandas as pd
import json
...
def get_current_weather(lat, lon):
    base_data = fetch_open_meteo(lat, lon)
...
def main():
    ...
    # --- v20.0: Windy Map Embed ---
    # デフォルトは風(wind)表示。明石にズーム。
    st.markdown("### 🗺️ 高度海況マップ (Windy Embed)")
    
    # Windy用HTML (オーバーレイ選択可能にするとさらに便利だが、まずは風を表示)
    # lat, lon は 現在地を使用
    windy_html = f"""
