...
def calc_strategy_realtime(wind_spd, wind_dir, tide_factor, is_rising, temp, cloud, rain):
    ...
    # --- ヘッド重量計算 (3パターン) ---
    # 係数計算: 基本1.2倍(TG前提で少し軽めだが潮圧考慮)
    # 激流(+0.5), 同調(+0.3), 強風(+0.2)
    multiplier = 1.1 # 基準(緩潮時)
    
    if tide_factor > 0.7: multiplier += 0.5 # 激流
    elif tide_factor > 0.3: multiplier += 0.2 # 適度
    
    if is_synced: multiplier += 0.3 # 船流れる
    if wind_spd > 7.0: multiplier += 0.2 # 強風
