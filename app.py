...
st.set_page_config(page_title="魔釣Pro - JCG Edition", page_icon="⚓️")
...
def get_jcg_tide_data():
    """海上保安庁の明石海峡潮流情報を取得する"""
    # URL: 第五管区海上保安本部 明石海峡潮流推算
    ...
    try:
        ...
        # ※JCGサイトのエンコーディング等は自動判定に任せる
        dfs = pd.read_html(url, encoding='shift_jis')
        
        # 最初のテーブルが予報データであることが多い
        if dfs:
            df = dfs[0]
            # カラム名などを調整して使いやすい形にする
            # JCGの表形式に合わせてパース
            return df
        return None
...
def parse_jcg_data(df, current_hour, current_min):
    """
    JCGのデータフレームから現在時刻の潮流情報を抽出する
    戻り値: (speed_knot, direction_text, is_real_data)
    """
    if df is None: return None, None, False
    
    try:
        # JCGの表は [時, 分, 流向, 流速(ノット)] の構成
        # カラム名を特定（表記揺れ対応のためインデックスで指定推奨）
        # 例: 0=時, 1=分, 2=流向, 3=流速
        
        # 現在時刻に近い行を探す
        # シンプルに線形補間は難しいので、直近のデータを採用
...
# --- 潮流計算 (ハイブリッド: スクレイピング失敗時は推測) ---
def get_hybrid_tide_data(moon_age, hour, minute):
    # 1. まずJCGデータをトライ
    df = get_jcg_tide_data()
    knot, dr_text, success = parse_jcg_data(df, hour, minute)
    
    if success:
        # JCGデータ取得成功
        tide_dir_deg = 280 if "西" in dr_text else 100 # 西なら280度、東なら100度
        is_rising = "西" in dr_text
        tide_factor = min(knot / 6.0, 1.0) # 最大6ktとして正規化
        return tide_factor, is_rising, knot, True # True=公式データ
    
    else:
        # 2. 失敗時は推測ロジック (バックアップ)
        tide_factor, is_rising, knot = estimate_tide_current_logic(moon_age, hour + minute/60)
        return tide_factor, is_rising, knot, False # False=推測データ
...
def main():
    st.markdown("""
        ...
        <p style='text-align: center; font-size: 14px; color: gray;'>
            JCG Official Data & Strategy v6.0
        </p>
    """, unsafe_allow_html=True)
    ...
    with st.spinner('海上保安庁のデータ照会・解析中...'):
        ...
        if data:
            ...
            # --- 潮流取得 (ハイブリッド) ---
            tide_factor, is_rising, real_knot, is_official = get_hybrid_tide_data(moon_age, now.hour, now.minute)
            ...
            # 潮流ラベルに公式か推測かを表示
            tide_label = "潮流 (JCG公式)" if is_official else "潮流 (推測値)"
            c2.metric(tide_label, knot_text, tide_display)
