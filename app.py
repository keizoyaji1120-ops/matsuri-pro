...
"""
            ...
            # v22.3: スマホレイアウト最適化用のヘッダー幅調整
            forecast_html = "<table class='forecast-table'><thead><tr><th style='width:12%;'>時間</th><th style='width:28%;'>天気/風</th><th style='width:24%;'>潮流(推測)</th><th style='width:24%;'>色(目安)</th><th style='width:12%;'>指数</th></tr></thead><tbody>"
            ...
            for i in range(1, 7):
                ...
                # 文字数削減: スラッシュより前、括弧より前の文字だけを抽出して改行処理
                short_color = f_rec_color.split(" / ")[0].split(" [")[0].replace(" (", "<br>(")
                
                forecast_html += f"<tr><td class='fc-time'>{day_str}{f_h}:00</td><td>{fw_text}</td><td>{ft_text}</td><td style='line-height:1.2;'><span style='font-size:10px; font-weight:bold; color:#d35400;'>{short_color}</span></td><td>{f_score_html}</td></tr>"
