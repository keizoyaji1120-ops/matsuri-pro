...
st.set_page_config(page_title="魔釣Pro - ScoreMaster", page_icon="⚓️")
...
st.markdown("""
    ...
    .seat-grid {
        ...
        text-align: center; border: 2px solid #2196f3; position: relative;
    }
    .seat-cell {
    ...
    .seat-best {
        background-color: #ffeb3b; border: 2px solid #fbc02d; color: #d32f2f;
        box-shadow: 0 0 10px rgba(255, 235, 59, 0.8); transform: scale(1.05);
    }
    .boat-shape {
        grid-column: 1 / -1; background-color: #607d8b; color: white;
        padding: 5px; border-radius: 50% 50% 5px 5px; margin-bottom: 10px; font-size: 12px;
    }
...
def calculate_best_seat(wind_dir, tide_dir_deg):
    ...
    if 337.5 <= relative_angle or relative_angle < 22.5:
        seat_name = "ミヨシ (船首)"; seat_code = "m_center"
    elif 22.5 <= relative_angle < 67.5:
        seat_name = "右ミヨシ"; seat_code = "m_right"
    elif 67.5 <= relative_angle < 112.5:
        seat_name = "右舷 (胴の間)"; seat_code = "c_right"
    elif 112.5 <= relative_angle < 157.5:
        seat_name = "右トモ"; seat_code = "t_right"
    elif 157.5 <= relative_angle < 202.5:
        seat_name = "トモ (船尾)"; seat_code = "t_center"
    elif 202.5 <= relative_angle < 247.5:
        seat_name = "左トモ"; seat_code = "t_left"
    elif 247.5 <= relative_angle < 292.5:
        seat_name = "左舷 (胴の間)"; seat_code = "c_left"
    elif 292.5 <= relative_angle < 337.5:
        seat_name = "左ミヨシ"; seat_code = "m_left"
...
def calculate_matsuri_score(tide_factor, is_synced, wind_spd, temp, rain):
    ...
    # 潮流要因 (明石は潮が命)
    if tide_factor > 0.7: score += 2.5 # 激流良し
    elif tide_factor > 0.4: score += 1.0
    elif tide_factor < 0.2: score -= 3.0 # 潮止まりは辛い
    
    # 同調要因
    if is_synced: score += 2.0 # 船が流れる＝広範囲探れる
    else: score -= 1.0 # 船が止まる＝バーチカルのみ
    
    # 風要因
    if 2.0 <= wind_spd <= 6.0: score += 1.0 # 最適風
    elif wind_spd > 8.0: score -= 2.0 # 荒れすぎ
    elif wind_spd < 1.0 and not is_synced: score -= 1.0 # 無風地獄
    
    # 水温・活性要因
    if temp > 15.0: score += 0.5
    elif temp < 10.0: score -= 1.0
    
    # 天気要因
    if rain > 0: score += 0.5 # 低気圧はプラス要素もある
...
def calc_strategy_realtime(wind_spd, wind_dir, tide_factor, is_rising, temp, cloud, rain):
    ...
    # --- ネクタイサイズ ---
    tie_size = "標準カーリー"
    if temp < 12.0: tie_size = "極細ショート / ストレート"
    elif tide_factor < 0.2: tie_size = "ショート / スリム"
    elif rain > 0.5: tie_size = "ワイド / 強波動"
    elif tide_factor > 0.8 and is_synced: tie_size = "ロングカーリー / ワイド"
    elif temp > 22.0: tie_size = "中厚カーリー"
    else: tie_size = "ショートカーリー"

    # --- ネクタイカラー ---
    color = "オレンジ / 赤オレ"
    if rain >= 0.5: color = "黒金 / チャート"
    elif cloud >= 80: color = "マジョーラゼブラ / 赤ゼブラ"
    elif 30 <= cloud < 80: color = "オレンジゼブラ / オレンジ"
...
def main():
    ...
    with st.spinner('釣果指数を算出中...'):
        ...
        if data:
            ...
            tide_factor, is_rising, est_knot = estimate_tide_current(moon_age, now.hour + now.minute/60)
            ...
            c1.metric("風向き", f"{wind_cardinal}", f"{wind_dir}°")
            c2.metric("潮流 (推測)", knot_text, tide_display)
            ...
            def get_style(target_code):
                base = "seat-cell"
                if target_code == seat_code: base += " seat-best"
                if target_code in ["m_center", "t_center"] and target_code == seat_code: return base 
                return base
            ...
            st.info(f"**【玄人解説】**\n現在、風は**{wind_cardinal}**から吹いており船首はその方向を向いています。\n潮流は**{tide_cardinal}方向**へ**{knot_text}**の速さで流れていると推測されます。")
    ...
    <div class="footer-box">
        ...
        <ul>
            ...
            <li>
                ...
                本アプリの提案は推測ロジックに基づくものであり、実際の海況や釣果を保証するものではありません。特に潮流のノット表示は明石海峡の月齢データを基にした推測値であり、実測値ではありません。
            </li>
            <li>
