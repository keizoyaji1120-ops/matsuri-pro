...
def get_current_weather(lat, lon):
    # v19.0: 日の出日の入りも取得 (daily=sunrise,sunset)
...
def fetch_open_meteo(lat, lon):
    ...
    # v19.0: daily=sunrise,sunset を追加
    params = {
        "latitude": lat,
        "longitude": lon,
...
def calc_strategy_realtime(wind_spd, wind_dir, tide_factor, is_rising, temp, cloud, rain, target_depth_mode, sunrise_dt, sunset_dt, current_dt):
    ...
    # --- カラー選定ロジック (v19.0: マズメ・時間帯判定) ---
    color = "オレンジ / 赤オレ"
    
    current_month = datetime.datetime.now().month
    is_nori_season = current_month in [11, 12, 1, 2, 3, 4]
    
    # 時間帯チェック (UTC->JST変換済みと仮定して比較)
    # マズメ: 日の出・日没の前後1時間
    is_mazume = False
    is_night = False
    
    if sunrise_dt and sunset_dt:
        # datetime比較のため tzinfo を合わせる
