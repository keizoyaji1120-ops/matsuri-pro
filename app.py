...
def get_current_weather(lat, lon):
    ...
    if sst_list and current_hour < len(sst_list) and sst_list[current_hour] is not None:
        has_sst = True
        base_data["sst_source"] = "local" # 成功: ピンポイント
    
    # 2. 取れていなければ「周辺サーチ」開始
    if not has_sst:
        # 探索するオフセット (順に: 南へ2km, 南へ5km, 南東, 南西)
        # 明石・瀬戸内は南側が沖合であることが多い
        search_offsets = [
            (-0.02, 0.00), # 南へ約2km
            (-0.05, 0.00), # 南へ約5km
            (-0.03, 0.03), # 南東へ
        ...
        for d_lat, d_lon in search_offsets:
            ...
            # API負荷軽減のため少しwaitを入れるのは実運用ではマナーだが、ここはStreamlitキャッシュ頼み
            ...
            if search_data:
                ...
                if s_sst_list and current_hour < len(s_sst_list) and s_sst_list[current_hour] is not None:
                    ...
                    base_data["sst_source"] = "search" # 成功: 周辺補完
                    has_sst = True
                    break # ループ終了
    
    if not has_sst:
        base_data["sst_source"] = "none" # 失敗: 統計値へ
        ...
        if data:
            ...
            for i in range(1, 5):
                ...
                if data["hourly"]["wind_speed_10m"] and len(data["hourly"]["wind_speed_10m"]) > target_idx:
                    ...
                    # 未来の水温: 未来予測は現在値(sst)を流用する(サーチが複雑になるため)
